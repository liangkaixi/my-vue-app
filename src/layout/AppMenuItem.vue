...
function handleClick(event) {
    ...
    emits('select', props.item.label);  // 向父组件发送 `select` 事件，传递 `label`
}
...
<template>
    <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
        ...
        <router-link v-if="item.to && !item.items && item.visible !== false" @click="handleClick"
        ...
        <Transition v-if="item.items && item.visible !== false" name="layout-submenu">
            <ul v-show="root ? true : isActiveMenu" class="layout-submenu">
                <app-menu-item v-for="(child, i) in item.items" :key="child" :index="i" :item="child"
                    :parentItemKey="itemKey" :root="false" @select="emits('select', $event)"></app-menu-item>
            </ul>
        </Transition>
    </li>
</template>
